    ...
    const locationRouterSection = form.addPageBreakItem()
      ...
      .setHelpText(
        'Your location is preselected from this assignment. Do not change it. If it is wrong, please flag the error to the leader.'
      );

    const locationRouterItem = form.addMultipleChoiceItem()
      .setTitle('Location — do not change')
      .setRequired(true);
    ...
    // ----------------------------------------------------------
    // CHANGE REPLACEMENT LOCATION ROUTER
    // ----------------------------------------------------------

    const changeLocationRouterSection = form.addPageBreakItem()
      .setTitle('Change Replacement Location')
      .setHelpText(
        'Your location is preselected from this assignment. Do not change it. If it is wrong, please flag the error to the leader.'
      );

      const changeLocationRouterItem = form.addMultipleChoiceItem()
        .setTitle('Change Replacement Location — do not change')
        .setRequired(true);


    // ----------------------------------------------------------
    // SECTION 6 — LOCATION-SPECIFIC NEW REPLACEMENTS
    // ----------------------------------------------------------

    const changeReplacementSections = {};
    ...
    activeLocations.forEach(location => {
      ...
      const section = form.addPageBreakItem()
        .setTitle('Select New Replacement — ' + location)
        .setHelpText(
          'Choose the facilitator you are now approaching and record the current status.'
        );

      const facilitatorItem = form.addListItem()
        .setTitle('New Proposed Replacement — ' + location)
        .setRequired(true);
      ...
      const statusItem = form.addMultipleChoiceItem()
        .setTitle('New Replacement Status — ' + location)
        .setRequired(true);

      changeReplacementSections[location] = {
        section: section,
        statusItem: statusItem
      };
    ...
    changeLocationRouterItem.setChoices(
      activeLocations.map(location =>
        changeLocationRouterItem.createChoice(
          location,
          changeReplacementSections[location].section
        )
      )
    );
    ...
    Object.values(replacementSections).forEach(({ section, statusItem }) => {
      ...
      followUpStatus.setChoices([
        ...
        followUpStatus.createChoice(
          ...
          changeLocationRouterSection
        )
      ]);

      // ----------------------------------------------------------
      // ROUTING: LOCATION-SPECIFIC SECTION 6
      // ----------------------------------------------------------

    Object.values(changeReplacementSections).forEach(
      ({ section, statusItem }) => {
    ...
    function verifyConfirmationFormDesign_() {
      ...
      activeLocations.forEach(location => {
        requiredQuestions.push(
          ...
          'New Proposed Replacement — ' + location,
    ...
    function buildPrefilledConfirmationUrl_(form, data) {
      ...
      .map(item => item.asMultipleChoiceItem())
      .find(item => item.getTitle() === 'Location — do not change');
      
      const changeLocationRouterItem = form
      .getItems(FormApp.ItemType.MULTIPLE_CHOICE)
      .map(item => item.asMultipleChoiceItem())
      .find(item => item.getTitle() === 'Change Replacement Location — do not change');

    if (!changeLocationRouterItem) {
      throw new Error('Change Replacement Location Router question not found.');
    }

    response.withItemResponse(
      changeLocationRouterItem.createResponse(data.location)
    );
    ...
    function getSessionLocation_(sessionId) {
      const sheet = SpreadsheetApp
        .openById(ROSTER_SPREADSHEET_ID)
        .getSheetByName(SESSIONS_SYNC_TAB);
      ...
      const row = rows
        .slice(1)
        .find(row =>
          row[sessionIdIndex].trim() === sessionId.trim()
        );
    ...
    function handleConfirmationSubmission(e) {
      ...
      .forEach(itemResponse => {
        ...
        if (title.startsWith('New Proposed Replacement — ')) {
          title = 'New Proposed Replacement';
        }
        
        if (title.startsWith('New Replacement Status — ')) {
          title = 'New Replacement Status';
        }
